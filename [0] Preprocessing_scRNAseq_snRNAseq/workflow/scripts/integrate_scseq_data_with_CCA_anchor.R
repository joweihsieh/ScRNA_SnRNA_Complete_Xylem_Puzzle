...
# Exclude one failed QC sample from integrating tree
# exclude_i_from_tree <- function(integrating_tree, excluded_id) {
#     working_tree_matrix <- cbind(integrating_tree, seq(nrow(integrating_tree)))

#     excluded_row_col <-
#         which(working_tree_matrix == excluded_id, arr.ind = TRUE)
#     replaced_hub <-
#         working_tree_matrix[excluded_row_col[1], 3]
#     orphan_node <-
#         working_tree_matrix[excluded_row_col[1], 3 - excluded_row_col[2]]

#     working_tree_matrix <- working_tree_matrix[-excluded_row_col[1], ]
#     working_tree_matrix[
#         which(working_tree_matrix == replaced_hub, arr.ind = TRUE)
#     ] <- orphan_node

#     for (i in seq(replaced_hub + 1, max(working_tree_matrix))) {
#         working_tree_matrix[
#             which(working_tree_matrix == i, arr.ind = TRUE)
#         ] %<>% add(-1)
#     }

#     for (i in seq(excluded_id - 1, min(working_tree_matrix), -1)) {
#         working_tree_matrix[
#             which(working_tree_matrix == i, arr.ind = TRUE)
#         ] %<>% add(1)
#     }

#     out <- working_tree_matrix[, -3]
#     return(out)
# }
...
integrate_and_clustering <- function(
    ...
    integration_anchors <- FindIntegrationAnchors(
        ...
        anchor.features = anchor, # JW corespondence
        #anchor.features = 2000, #Tung; orginal
        scale = TRUE,
        reduction = "cca",
        l2.norm = TRUE,
        #k.anchor = 5,
...
anchor <- as.numeric(snakemake@wildcards$anchor)
filters <- as.numeric(snakemake@wildcards$filters)
k_anchor <- as.numeric(snakemake@wildcards$k_anchor)


# input_multi_UMI_csv <- paste0(
#     "results/Ortholog_analysis/all_data_tables/orthologUMI_",
#     c(
...
#     ".csv"
# )
# integrating_order <- c(-2, -1, -4, -3, 1, 2, -5, -6, 4, -7, 5, -8, 6, -9, 3, 7)
# k_param <- 3
...
is_pass <-
    sapply(
        ...
        function(UMI_object) {
            ...
            #could be used in v4, v5
            cm <- GetAssayData(UMI_object, assay = "RNA", slot = "counts")
            n_feature <- nrow(cm)
            n_barcode <- ncol(cm)
...
# sample_vector <- names(UMI_list)
# excluded_sample <- sample_vector[!is_pass]
# if (length(excluded_sample) > 0) {
#     for (exsam in excluded_sample) {
#         exid <- -which(sample_vector == exsam)
#         sample_vector <- sample_vector[exid]
#         integrating_tree <-
#             exclude_i_from_tree(
#                 integrating_tree,
#                 excluded_id = exid
